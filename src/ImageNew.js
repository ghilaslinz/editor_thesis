...
import anchor from 'suneditor/src/plugins/modules/_anchor.js';
...
const image_new = {
    ...
    setDialog: function (core) {
        ...
        let html = '' +
            ...
            '</div>' +
            '<div class="se-dialog-tabs" style="display: none">' +
            '<button type="button" class="_se_tab_link active" data-tab-link="image" style="display: none" >' + lang.toolbar.image + '</button>' +
            '<button type="button" class="_se_tab_link" data-tab-link="url" style="display: none">' + lang.toolbar.link + '</button>' +
            '</div>' +
        ...
        if (option.imageUrlInput) {
            html += '' +
                '<div class="se-dialog-form">' +
                '<label for="url">' + lang.dialogBox.imageBox.url + '</label>' +
                '<div class="se-dialog-form-files">' +
                '<input id="url" class="se-input-form se-input-url _se_image_url" type="text"  />' +
        ...
        html += '</div>' +
            '<div class="se-dialog-form">' +
            '<label for="alt">' + lang.dialogBox.imageBox.altText + '</label><input id="alt" class="se-input-form _se_image_alt" type="text" />' +
            '</div>';
        ...
        if (option.imageResizing) {
            ...
            if (onlyPercentage || !option.imageHeightShow) {
                ...
                html += '' +
                    '<div class="se-dialog-size-text" style="display: none">' +
            ...
            html += '' +
                '<input style="display: none" class="se-input-control _se_image_size_x" placeholder="auto"' + (onlyPercentage ? ' type="number" min="1"' : 'type="text"') + (onlyPercentage ? ' max="100"' : '') + ' />' +
                '<label style="display: none" class="se-dialog-size-x"' + heightDisplay + '>' + (onlyPercentage ? '%' : 'x') + '</label>' +
                '<input style="display: none" type="text" class="se-input-control _se_image_size_y" placeholder="auto"' + onlyPercentDisplay + (onlyPercentage ? ' max="100"' : '') + heightDisplay + '/>' +
                '<label' + onlyPercentDisplay + heightDisplay + ' style="display: none"><input type="checkbox" class="se-dialog-btn-check _se_image_check_proportion" checked style="display: none"/>&nbsp;' + lang.dialogBox.proportion + '</label>' +
                '<button style="display: none" type="button" title="' + lang.dialogBox.revertButton + '" class="se-btn se-dialog-btn-revert" style="float: right;">' + core.icons.revert + '</button>' +
                '</div>';
        }

        html += '' +
            '<div class="se-dialog-form se-dialog-form-footer"  >' +
            '<label style="display: none"><input checked type="checkbox" class="se-dialog-btn-check _se_image_check_caption "/>&nbsp;' + lang.dialogBox.caption + '</label>' +
            '</div>' +
            '</div>' +
            '</div>' +
    ...
    create_image: function (src, anchor, width, height, align, file) {
        ...
        if (contextImage._captionChecked) {
            ...
            contextImage._caption.setAttribute('contenteditable', true);
    ...
    init: function () {
        ...
        contextImage.captionCheckEl.checked = true;
